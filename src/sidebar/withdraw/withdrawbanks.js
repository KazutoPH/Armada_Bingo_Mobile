...
class WithdrawBanks extends Component {
	state = {
		bankdetails: [],
		value: '',
		errors: '',
	...
	componentDidMount = async () => {
		...
		const withdrawbanks = await this.props.withdrawbanks.banks;
		await this.setState({
	...
	submit = async () => {
		...
		if (!!validata) {
			...
			if (parseFloat(this.state.amount) + parseFloat(this.state.fee) > parseFloat(this.props.balance)) {
				Toast.hide();
				...
				const obj = {
					...
					LastName: this.state.lastname,
					Amount: parseFloat(this.state.amount) + parseFloat(this.state.fee),
					ProcId: this.state.bankname,
					ProcDetail: this.state.accountnumber,
					page: 'withbanks',
					phone: this.props.auth.phone
				};
				await this.setState({ formdata: obj });
				Toast.hide();
		...
		return (
			<SafeAreaView style={{ flex: 1 }}>
				<View style={styles.container}>
					<MainHeader notifications={false} menu={false} onPress={() => Actions.pop()} title='Select Bank' />
						...
						<ScrollView style={{ marginBottom: hp('15%') }}>
							...
							{this.state.bankform ? (
								<View>
									...
									>
										<Image style={styles.circleContainer} source={{ uri: this.state.banklogo }} />
									</View>
									<ScrollView>
										...
										<View style={styles.buttonContainer}>
											<View style={styles.signinwrapper}>
												{!this.state.loader ? (
													...
													>
														<BarIndicator color='#353535' count={6} />
													</View>
												)}
											</View>
										</View>
...
export default connect(mapStateToProps)(WithdrawBanks);
